...
var knownExperimentalFeatures = map[string]string{
	...
	tools.DataQualityFeatureName:       "Data quality tools: create data quality jobs (create_data_quality_job: discovery + preview + create in one); cancel an in-progress job run (dq_cancel_job_run); delete a completed job run and its results (dq_delete_job_run); create, validate, read and search rules; per-run rule results; rule templates (list, read, deploy); plain-language (Text2SQL) rule generation; and catalog column search.",
}
