...
- `COLLIBRA_MCP_SKILLS_DIR` - Optional path to an external skills directory. When set, its skills are merged on top of the embedded catalog and same-named skills (e.g. `collibra/lineage`) fully replace the embedded entry. Requires the `skills` experimental feature. `~` and `~user` are expanded. A skill may declare `requires: <capability>` in its frontmatter to be served only when that capability is on (`data-quality` is the only one today); an unrecognized value fails catalog load and the server does not start, so a capability rename is a breaking change for such a skill.
...
- `skills-dir` - optional path to an external skills directory whose contents merge on top of the embedded catalog. Same-named skills fully replace the embedded entry. Requires the `skills` experimental feature. `~` and `~user` are expanded. A skill's `requires: <capability>` frontmatter gates it on a capability flag; an unrecognized value fails catalog load and the server does not start.
...
export COLLIBRA_MCP_API_USR="your-username"
export COLLIBRA_MCP_API_PWD="your-password"
export COLLIBRA_MCP_ENABLED_TOOLS="search_asset_keyword,get_asset_details,list_asset_types"
./mcp-server
```
