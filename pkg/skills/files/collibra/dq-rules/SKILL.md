...
Rule tools: `validate_dq_rule`, `create_dq_rule`, `get_dq_rule`, `get_dq_rule_results`.
...
1. **Validate before you create.** Always call `validate_dq_rule` on the rule SQL *before*
   ...
   front. `validate_dq_rule` takes the raw SQL — the rule does **not** need to exist yet. If
...
2. **`validate_dq_rule` needs discovery IDs.** It requires `edgeSiteId`, `connectionId` and
   `schemaName`. Get them from `prepare_create_dq_job` for the target job — do not guess them.
   (`create_dq_rule`, `get_dq_rule` and `get_dq_rule_results` take only names/ids and need no
   discovery step.)
...
6. **Rules require a PUSHDOWN job.** If `create_dq_rule` returns an error mentioning the dataset
...
7. **Read the `status` field in every response.** Branch on `success`, `validation_error`, or
   `error`. For `validate_dq_rule`, `status: success` means validation *ran* — the verdict is
...
8. **Creating a rule does not run it.** A new rule is only evaluated on the job's next run
   ...
   happened, use `get_dq_rule_results` to see how the rule did.

## Workflow: author a rule

1. **Discover** — call `prepare_create_dq_job` for the job to get `edgeSiteId`, `connectionId`
...
`get_dq_rule` returns a rule's current definition (type, SQL, filter, tolerance,
