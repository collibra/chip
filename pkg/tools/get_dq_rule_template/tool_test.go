...
func server(t *testing.T, code int, tmpl clients.DQRuleTemplate) *http.Client {
	...
	mux.HandleFunc("GET /rest/dq/1.0/ruleTemplates/{ruleTemplateName}", func(w http.ResponseWriter, r *http.Request) {
...
func TestGetDQRuleTemplate_HappyPath(t *testing.T) {
	c := server(t, http.StatusOK, clients.DQRuleTemplate{ID: "t1", Name: "Not Null Check", IsSystem: true, DeployedRuleCount: 3})
	out, err := get_dq_rule_template.NewTool(c).Handler(t.Context(), get_dq_rule_template.Input{RuleTemplateName: "Not Null Check"})
	if err != nil {
	...
	if out.Template == nil || out.Template.Name != "Not Null Check" || out.Template.DeployedRuleCount != 3 {
...
func TestGetDQRuleTemplate_NotFound(t *testing.T) {
	...
	out, _ := get_dq_rule_template.NewTool(c).Handler(t.Context(), get_dq_rule_template.Input{RuleTemplateName: "nope"})
