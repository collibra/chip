package get_column_semantics_test
...
import (
	...
	tools "github.com/collibra/chip/pkg/tools/get_column_semantics"
...
func TestNonexistentColumnIsAnError(t *testing.T) {
	...
	_, err := tools.NewTool(testutil.NewClient(server)).Handler(t.Context(), tools.Input{ColumnID: missingID})
	if err == nil {
		t.Fatal("expected an error for a nonexistent column, got a success payload")
	}
...
func TestExistingColumnWithoutRelationsSucceeds(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/rest/2.0/relations", emptyRelations)
	mux.HandleFunc("/rest/2.0/assets/"+realID, func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"id":"` + realID + `","name":"Example","type":{"id":"00000000-0000-0000-0000-000000031008","name":"Column"}}`))
	})

	server := httptest.NewServer(mux)
	defer server.Close()

	output, err := tools.NewTool(testutil.NewClient(server)).Handler(t.Context(), tools.Input{ColumnID: realID})
	if err != nil {
		t.Fatalf("expected no error for a real column with nothing connected, got: %v", err)
	}
	if len(output.Semantics) != 0 {
		t.Fatalf("expected an empty result, got %d", len(output.Semantics))
	}
}
