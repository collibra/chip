package get_table_semantics_test
...
import (
	...
	tools "github.com/collibra/chip/pkg/tools/get_table_semantics"
...
func TestNonexistentTableIsAnError(t *testing.T) {
	...
	_, err := tools.NewTool(testutil.NewClient(server)).Handler(t.Context(), tools.Input{TableID: missingID})
	if err == nil {
		t.Fatal("expected an error for a nonexistent table, got a success payload")
	}
...
func TestExistingTableWithoutRelationsSucceeds(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/rest/2.0/relations", emptyRelations)
	mux.HandleFunc("/rest/2.0/assets/"+realID, func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"id":"` + realID + `","name":"Example","type":{"id":"00000000-0000-0000-0000-000000031007","name":"Table"}}`))
	})

	server := httptest.NewServer(mux)
	defer server.Close()

	output, err := tools.NewTool(testutil.NewClient(server)).Handler(t.Context(), tools.Input{TableID: realID})
	if err != nil {
		t.Fatalf("expected no error for a real table with nothing connected, got: %v", err)
	}
	if len(output.SemanticHierarchy) != 0 {
		t.Fatalf("expected an empty result, got %d", len(output.SemanticHierarchy))
	}
}
