...
type Input struct {
	ConnectionID string `json:"connectionId" jsonschema:"UUID of the connection to test (created via create_connection)."`
	TimeoutSec   int    `json:"timeoutSec,omitempty" jsonschema:"Optional. If provided and greater than 0, waits synchronously for the test to finish (up to this many seconds) and returns the final result. If omitted, returns immediately with a jobId while the test runs in the background — poll it with get_job_status."`
}

type Output struct {
	JobID   string `json:"jobId,omitempty" jsonschema:"The id of the connection-test job. Use with get_job_status to poll for the result if timeoutSec was not provided."`
	Success bool   `json:"success" jsonschema:"Whether the test connection request itself succeeded. If timeoutSec was omitted, this reflects that the test job was submitted, not the connection test's outcome — check get_job_status for that."`
