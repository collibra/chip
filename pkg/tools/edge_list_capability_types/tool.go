// Package edge_list_capability_types implements the edge_list_capability_types MCP tool: a
// read-only discovery tool for finding valid typeId values (and their manifests) for
// edge_create_capability and edge_create_connection on a given edge site.
package edge_list_capability_types
...
type Input struct {
	EdgeSiteID string `json:"edgeSiteId" jsonschema:"UUID of the edge site to list available capability and connection types for. Use the edge_list_sites tool to discover available sites."`
...
func NewTool(collibraClient *http.Client) *chip.Tool[Input, Output] {
	return &chip.Tool[Input, Output]{
		Name:        "edge_list_capability_types",
