...
- [`create_asset_access_request`](pkg/tools/create_asset_access_request/) - Create a Collibra Data Access request for a catalog asset. The WHAT of the request is the Data Access role linked to the asset, which the tool resolves — never pass a role or a data object. Required inputs: `assetId`, `purpose`, `expiresAt`, and at least one beneficiary in `users` and/or `groups`. Optional: `name` and `outputPortId`:
    - **Which assets can be requested** - any asset that has an active Grant linked to it. This is instance configuration, so the roles actually linked to the asset are read rather than inferred from its type. A Data Product with no role of its own is expanded to its output ports, since that is normally where its role lives; naming a port in `outputPortId` always takes precedence over the product itself
    - **Who** - `users` takes Collibra email addresses (or usernames), mapped to Data Access users by email; `groups` takes Collibra group names (or UUIDs), mapped to Data Access groups by name. At least one beneficiary is required and both can be combined. Nothing is created unless every entry maps — failures come back in `unresolvedUsers` / `unresolvedGroups`, including a group name that matches several Data Access groups
    - **Purpose** - `purpose` is mandatory and must come from the user; it is the business justification, used verbatim as the request's description with a note appended stating it was created by AI, and it is what the suggested name is derived from. An empty or blank purpose is refused before anything is looked up
    - **When it expires** - `expiresAt` is mandatory; a plain date (`2026-12-31`) is taken as the end of that day UTC, and an RFC 3339 timestamp is used as given
    - **`no_role_linked`** - the asset has no requestable role. `linkedRoles` carries what is linked, if anything, so the caller can say whether nothing is linked at all or what is linked is not an active Grant
    - **`needs_port_selection`** - the Data Product has several output ports, returned in `outputPorts`; ask the user which one and call again with `outputPortId`
    - **`needs_name_confirmation`** - no `name` was supplied; confirm the returned `suggestedName` with the user and call again
    - **`created`** - the request exists. The response carries the asset, the role it was requested through, the mapped users and groups, the expiration date, and a link to the request in Collibra
