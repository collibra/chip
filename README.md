...
> **Gating:** tools flagged with **Gated:** are only registered when the named capability flag is on — they are absent from `tools/list` otherwise. Today the only such capability is `data-quality`, enabled with `--data-quality` (see [Data quality tools](#data-quality-tools)).
...
- [`get_data_quality_rule`](pkg/tools/get_dq_rule/) - Read the definition of a single DQ rule (monitor) on a job — its type, SQL, filter, tolerance and active/suppressed state. **Gated:** the `data-quality` capability (`--data-quality`)
- [`get_data_quality_rule_results`](pkg/tools/get_dq_rule_results/) - Read a rule's per-run results after a job run — score, breaking/passing record counts, pass/fail status and any exception. Paginated (`offset`/`limit`), newest first by default. **Gated:** the `data-quality` capability (`--data-quality`)
- [`validate_data_quality_rule`](pkg/tools/validate_dq_rule/) - Validate a rule's SQL/definition against the source before saving or running it, so a malformed rule is caught up front. Returns whether the rule is valid plus the engine's message. Requires `edgeSiteId`/`connectionId`/`schemaName` identifying the DQ connection the rule runs against (connection discovery lives in `create_data_quality_job`). **Gated:** the `data-quality` capability (`--data-quality`)
- [`list_data_quality_rule_templates`](pkg/tools/list_dq_rule_templates/) - List the DQ rule templates (built-in + custom) available in the connected environment via the public DQ API. Each is a parameterized SQL pattern deployable via `deploy_data_quality_rule_template` (by its `ruleTemplateName`). Filters: `name`, `dimension`, `isSystem` (built-in vs custom); paginated. **Gated:** the `data-quality` capability (`--data-quality`)
- [`get_data_quality_rule_template`](pkg/tools/get_dq_rule_template/) - Read a single DQ rule template by `ruleTemplateName` — its parameterized SQL, dialect, dimensions, default tolerance, built-in (`isSystem`) flag and deployed-rule count. **Gated:** the `data-quality` capability (`--data-quality`)
- [`find_data_quality_rules`](pkg/tools/find_dq_rules/) - Search existing DQ rules (monitors) across jobs. Filter by exact `jobName` and/or `columnName` (combine both to detect rules already on a target column) or a rule-name substring. Returns each rule's job, column, type, status and SQL; paginated. **Gated:** the `data-quality` capability (`--data-quality`)
- [`generate_data_quality_rule_sql`](pkg/tools/generate_dq_rule_sql/) - Turn a plain-language description of a check into rule SQL (Text2SQL / Collibra DQ AI), so a rule can be authored without writing SQL. Returns a single SQL string (validate before use). Requires `edgeSiteId`/`connectionId` identifying the DQ connection (connection discovery lives in `create_data_quality_job`). **Gated:** the `data-quality` capability (`--data-quality`)
...
The data quality tools — data quality jobs, rules and rule templates — are **off by default**. They
are generally available, not experimental, but several write to Collibra and two
(`dq_delete_job`, `dq_delete_job_run`) delete irreversibly, so the capability is opted into as a
whole. Enable it with the `--data-quality` flag, the `COLLIBRA_MCP_DATA_QUALITY` environment
variable, or the `mcp.data-quality` field in `mcp.yaml`.

With the capability off, the 18 tools marked **Gated:** above are not registered at all: they do
not appear in `tools/list` and cannot be called.

Two consequences worth knowing:

- **`enabled-tools` cannot re-open the gate.** The allow-list filters the tools of the capabilities
  you have enabled; it is not an escape hatch around them. Naming `create_data_quality_rule` in
  `enabled-tools` without `--data-quality` registers nothing.
- **The data quality skills follow the flag.** With the `skills` experimental feature on, the
  `collibra/dq-rules` and `collibra/dq-rule-workbench` skill guides are served only when
  `--data-quality` is on, so an agent is never handed a guide for tools that are not registered.
  A skill declares this itself with `requires: data-quality` in its frontmatter, including a skill
  supplied through `--skills-dir`.

This capability flag is a separate axis from `--experimental`: `data-quality` is **not** a valid
