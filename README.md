...
- [`start_collibra_workflow`](pkg/tools/start_collibra_workflow/) - Discover, inspect, and start any Collibra workflow (OOTB or a customer-built custom workflow), identified by its `workflowDefinitionId` — there is no allowlist. Call with no `workflowDefinitionId` to discover workflows the current user is authorized to start (instance-wide, or scoped to an asset via `businessItemId`) — authorization filtering is best-effort: it hard-excludes only a confirmed-missing `WORKFLOW_START` permission, and annotates rather than hides options where role inheritance can't be fully verified. Call again with a `workflowDefinitionId` to preview it; if it requires a start form, returns `needs_input` with the field schema (`needs_input` also covers a missing `businessItemId` for resource-scoped workflows) — re-call with `formProperties` populated. Confirm checkpoint: `confirm=false` (default) returns a preview without starting anything; `confirm=true` starts it and returns the `workflowInstanceId`. **Experimental** (`workflows` feature flag)
...
- `workflows` — `start_collibra_workflow`: discover, inspect (including start-form fields), and start any Collibra workflow (OOTB or custom) by `workflowDefinitionId`, filtered to what the current user is authorized to start. WRITES to Collibra (starts a workflow instance), so it stays behind this flag until it graduates.
