...
- [`dq_get_job`](pkg/tools/get_dq_job/) - Read the full definition of a single Collibra data-quality job, by `name` or by the table it's configured on — type (PUSHDOWN/PULLUP), edge site, connection, schema/table, source SQL, run-date window, configured monitors (adaptive + custom DQ rules), notifications, and schedule. By `name`, an exact match is tried first; if none is found, jobs whose name contains the given text are offered as candidates (`needs_input`) to disambiguate. By `tableName` (optionally narrowed by `schemaName`), matching the job's data-location table rather than its free-text job name — if several jobs are on same-named tables in different locations, their schema/data source/connection are returned as candidates (`needs_input`) so you can pick one and re-call with its `jobName`. Read-only.
