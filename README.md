...
- [`list_workflow_definitions`](pkg/tools/list_workflow_definitions/) - Discover workflow definitions — the templates `start_workflow` starts instances from. Requires exactly one of `assetId`/`domainId`/`communityId` (workflows applicable there AND that the current user is authorized to start, both checked server-side) or `global=true` (every global workflow the user is permitted to start, including ones normally triggered by a timer or another workflow); always enabled-only. Also filters by `nameContains` (case-insensitive, matches the name or the start-button label); returns `startLabel` when it differs from the name; paginated (`total`/`hasMore`); typed `status` (`success`/`validation_error`/`error`) with a self-explaining `message`. **Experimental** (`workflows` feature flag)
...
- [`start_workflow`](pkg/tools/start_workflow/) - Inspect a workflow definition's start-form requirements and start a new instance, by `workflowDefinitionId` (from `list_workflow_definitions`). Handles both of Collibra's start-form models transparently. Returns `needs_input` for a missing `businessItemId` or missing/invalid form fields (with the allowed options for fixed-choice fields); a field needing a real Collibra resource is flagged `resourcePicker` rather than guessed, multi-value fields and server-supplied option shortlists are surfaced, and a field this tool cannot answer at all (file upload, role-in-community, or a picker whose form stores the whole resource rather than its id) says so instead of sending the caller on an impossible lookup. A value for a read-only field is refused before anything is written, and a `formProperties` key matching no field is flagged as a likely typo while still being sent. Confirm checkpoint: `confirm=false` (default) returns a preview without starting anything; `confirm=true` starts it and returns the `workflowInstanceId`. Who can actually start a given workflow is enforced by Collibra itself at start time. **Experimental** (`workflows` feature flag)
