...
- [`dq_run_job`](pkg/tools/run_dq_job/) - Trigger a run of an existing Collibra data-quality job AS-IS by `jobName`, with no changes to the job's definition. An exact name match is tried first; if none is found, jobs whose name contains the given text are offered as candidates (`needs_input`) to disambiguate. By default the run uses the job's current settings (current run date, no backfill); optionally override the run's time slice with `runDateKind`/`runDateValue` and `runDateEndKind`/`runDateEndValue` (`DATE` = yyyy-MM-dd, `TIMESTAMP` = RFC3339), or queue runs for prior periods with `backrunTimeBin` (`DAY`/`MONTH`/`YEAR`) + `backrunBinValue`. Safety checkpoint: `confirm=false` (default) is READ-ONLY — it resolves the job and returns a preview of exactly what would be queued (`runPlan`) without queueing anything; call again with `confirm=true` to start the run. Queueing a run consumes compute on the job's edge site, and a backfill can queue many runs at once. On success returns the new `jobRunId` — check it with `dq_get_job_run` or stop it with `dq_cancel_job_run`. To change a job's definition rather than run it, use `dq_update_job`. Requires the Data Quality Admin or Data Quality User global role (`DATA_QUALITY_JOB_RUN`). **Experimental** (`data-quality-experimental` feature flag)
