...
- **For RICH_TEXT attributes, write Markdown.** Both `create_asset` and `edit_asset` detect `RICH_TEXT` attributes (e.g. `Definition`) and convert Markdown to HTML server-side before writing. Use bold (`**...**`), links (`[text](url)`), bullet lists, and headings naturally; they render correctly in the Collibra UI. Plain-text attributes pass through unchanged. On `edit_asset` results, the `convertedFromMd` flag and `writtenValue` (HTML form) confirm what was actually submitted.
